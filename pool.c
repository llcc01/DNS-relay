...
void pool_id_free()
{
    pthread_mutex_destroy(&pool_id_mutex);
}

inline uint8_t pool_id_is_full()
{
    return pool_id_count == pool_id_size;
}
...
int32_t pool_id_get()
{
    ...
    pool_id_count++;
    uint16_t pool_id = pool_ids[(pool_id_base_index + pool_id_count) & (pool_id_size - 1)];
    pthread_mutex_unlock(&pool_id_mutex);
    return pool_id;
}
