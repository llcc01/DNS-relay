...
void dns_record_print(const dns_record_t *record) {
  ...
  if (record->type == TYPE_A) {
    ...
    printf("MX %d %s\n", record->rdata[0] << 8 | record->rdata[1],
           record->rdata + 2);
...
void dns_record_to_buf(const dns_record_t *record, uint8_t *buf, size_t *len,
                       size_t offset, size_t name_offset,
                       size_t *cname_offset) {
  ...
  } else if (*cname_offset != 0 &&
             strcmp((char *)&(buf[*cname_offset]), record->name) == 0) {
...
void dns_record_from_buf(const uint8_t *buf, size_t buf_len, size_t *len,
                         size_t offset, dns_record_t *record) {
...
void protocol_init(SOCKET *s, uint16_t port) {
  ...
  *s = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
  ...
  WSAIoctl(*s, SIO_UDP_CONNRESET, &bNewBehavior, sizeof(bNewBehavior), NULL, 0,
...
// 协议发送消息
void protocol_send(SOCKET s, const SOCKADDR_IN *sock_in,
  ...
  if (buffer_size > 512) {
    LOG_WARN("send buffer_size: %zu", buffer_size);
  }

  // QueryPerformanceCounter(&start);

  int res = sendto(s, (char *)buffer, buffer_size, 0, (SOCKADDR *)sock_in,
                   sizeof(*sock_in));
  free(buffer);
  if (res <= 0) {
...
void protocol_recv(SOCKET s, SOCKADDR_IN *sock_in, dns_message_t *msg) {
  ...
  int res = recvfrom(s, (char *)buffer, BUF_MAX_SIZE, 0, (SOCKADDR *)sock_in,
...
void dns_header_set_flags(dns_header_t *header, uint16_t flags, uint8_t opcode,
                          uint8_t rcode) {
...
void dns_header_from_buf(const uint8_t *buf, size_t buf_len,
                         dns_header_t *header) {
...
void dns_message_to_buf(const dns_message_t *msg, uint8_t *buf, size_t *len) {
  ...
  for (size_t i = 0; i < msg->header.ancount; i++) {
    ...
    dns_record_to_buf(&msg->answers[i], buf, &record_len, offset, name_offset,
                      &cname_offset);
    offset += record_len;
  }
  ...
  for (size_t i = 0; i < msg->header.nscount; i++) {
    ...
    dns_record_to_buf(&msg->authorities[i], buf, &record_len, offset,
  ...
  for (size_t i = 0; i < msg->header.arcount; i++) {
    size_t record_len;
    dns_record_to_buf(&msg->additionals[i], buf, &record_len, offset,
                      name_offset, &cname_offset);
    offset += record_len;
  }
...
void dns_message_from_buf(const uint8_t *buf, size_t buf_len,
                          dns_message_t *msg) {
  ...
  for (size_t i = 0; i < msg->header.qdcount; i++) {
    ...
    size_t question_len;
    dns_question_from_buf(buf, buf_len, &question_len, offset,
                          &msg->questions[i]);
    offset += question_len;
  }
...
uint8_t dns_message_is_empty(const dns_message_t *msg) {
  return msg->header.qdcount == 0 && msg->header.ancount == 0 &&
         msg->header.nscount == 0 && msg->header.arcount == 0;
}

void dns_question_to_buf(const dns_question_t *question, uint8_t *buf,
                         size_t *len) {
...
void dns_question_from_buf(const uint8_t *buf, size_t buf_len, size_t *len,
                           size_t offset, dns_question_t *question) {
...
// 解压缩name
void decompress_name(const uint8_t *buf, size_t buf_len, size_t offset,
